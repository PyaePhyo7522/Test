...
def registered_swimmers():
    ...
    #print("%15s %15s %15s %15s" % ("Name", "AGE", "Gender", "Status"))
    for each_row in zip(*([i] + (j) for i, j in  register_swimmers_table.items())):
          print("%15s %15s %15s %15s" % (each_row[0],each_row[1],each_row[2], "Status" if each_row[3] == "Status"  else "active" if each_row[3] else "inactive"))
...
def recorded_swimmers():
    ...
    for each_row in zip(*([i] + (j) for i, j in  record_swimmers_temp_table.items())):
         print("%9s %9s %9s %9s %9s %9s %9s %9s" % (each_row[0],each_row[1],each_row[2], ("Status" if each_row[3] == "Status"  else "active" if each_row[3] else "inactive"), each_row[4], each_row[5], each_row[6], each_row[7] ))                      
...
def real_swimmers():
    ...
    for each_row in zip(*([i] + (j) for i, j in   real_swimmer_table.items())):
         print("%9s %9s %9s %9s %9s %9s %9s %9s" % (each_row[0],each_row[1],each_row[2], ("Status" if each_row[3] == "Status"  else "active" if each_row[3] else "inactive"), each_row[4], each_row[5], each_row[6], each_row[7] ))          
    main_program = True
...
def search_swimmers(search_results):
    ...
    for each_row in zip(*([i] + (j) for i, j in   search_results.items())):
         print(*each_row, "")             
...
while main_program == True:
    user_choice = input("\n\n\nPress 1 for registering swimmers. \nPress 2 for remove swimmers\nPress 3 to record the swimmers\nPress 4 to search by name\nPress 5 to search by event \nPress 6 for unposted swimmers\nPress 7 to display posted swimmers\nPress 8 for registered swimmers.\nTo execute program press            => 9\n\n******************************************************************\n\nEnter Your Option :: ")
